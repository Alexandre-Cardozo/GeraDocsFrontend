...
import { Button, Dropdown, Input, StatusBadge, Tag, Textarea, ValidationMsg } from "@/components/ui"
import {
  IconCalendar,
  IconCheck,
  IconCheckCircle,
...
import { PAPEL_LABEL, STATUS_PROCESSO_LABEL, type ItemAprovacao, type TipoDocumento } from "@/lib/types"
...
export default function Aprovacoes() {
  ...
  const handleDecisao = (decisao: "aprovar" | "rejeitar") => {
    ...
    decidir.mutate(
      { processoId: active.processoId, decisao, comentario },
      {
        ...
        onError: (e) => showToast(e instanceof Error ? e.message : "Não foi possível registrar a decisão."),
      }
    )
  }
  ...
  const adicionarApontamento = () => {
    ...
    setApontamentos((prev) => [...prev, { tipo: apTipo, secaoTitulo: apSecao.trim(), texto: apTexto.trim() }])
    setApTipo("")
    setApSecao("")
    setApTexto("")
  }
  ...
  const confirmarRetificacao = () => {
    ...
    if (apontamentos.length === 0) {
      showToast("Adicione ao menos um apontamento de retificação antes de confirmar.")
      return
    }
    const comentario = comment.trim()
    ...
    decidir.mutate(
      {
        ...
        decisao: "retificar",
        comentario,
        apontamentos: lista.map((a) => ({ tipo: a.tipo, secaoTitulo: a.secaoTitulo || undefined, texto: a.texto })),
      },
      {
        onSuccess: () => showToast("Retificação solicitada — apontamentos registrados e processo devolvido."),
        onError: (e) => showToast(e instanceof Error ? e.message : "Não foi possível solicitar a retificação."),
      }
    )
  }
  ...
  const handleParecer = (favoravel: boolean) => {
    ...
    registrarParecer.mutate(
      ...
      {
        onSuccess: () =>
          showToast(favoravel ? "Parecer jurídico favorável registrado." : "Parecer jurídico desfavorável registrado."),
      }
    )
  }
  ...
  const handleEncaminhar = () => {
    ...
    encaminhar.mutate(
      ...
      {
        onSuccess: () => showToast("Processo encaminhado para decisão do gestor."),
        onError: (e) => showToast(e instanceof Error ? e.message : "Não foi possível encaminhar o processo."),
      }
    )
  }
  ...
  const handleConcluir = () => {
    ...
    concluir.mutate(
      { processoId: active.processoId, comentario },
      {
        onSuccess: () => showToast("Processo concluído — contratação homologada."),
        onError: (e) => showToast(e instanceof Error ? e.message : "Não foi possível concluir o processo."),
      }
    )
  }
    ...
    <div className="flex flex-col lg:h-full lg:flex-row lg:overflow-hidden">
      ...
      <div className="flex w-full shrink-0 flex-col overflow-hidden border-b border-border bg-surface lg:w-85 lg:min-w-85 lg:border-r lg:border-b-0">
        <div className="border-b border-border-soft px-4.5 py-4">
          <div className="text-base font-bold text-text-1">Fila de Aprovação</div>
          <div className="mt-0.5 text-sm text-text-3">{pendentesCount} aguardando decisão do gestor</div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {itens.length === 0 && <EmptyState message="Nenhum processo no fluxo de aprovação" />}
          {itens.map((a) => (
            <div
              key={a.processoId}
              onClick={() => {
            ...
            >
              <div className="mb-1.5 flex items-start justify-between gap-2">
                ...
                {a.status === "aguardando" && !a.urgente && <StatusBadge status={a.status} size="sm" />}
                {a.status !== "aguardando" && <StatusBadge status={a.status} size="sm" />}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                {a.documentos.map((tipo) => (
                  <span key={tipo} className={`rounded-sm px-1.5 py-0.5 font-mono text-2xs font-bold ${CATALOGO[tipo].chip}`}>
                    {tipo}
                  </span>
                ))}
              </div>
            ...
            <div className="flex flex-wrap items-start justify-between gap-2.5">
              <div className="min-w-0">
                <div className="mb-1 flex flex-wrap items-center gap-2">
                  <span className="font-mono text-xs text-text-muted">{active.processoId}</span>
                ...
                <div className="mt-2 flex flex-wrap gap-3.5">
                  ...
                  <span className={`inline-flex items-center gap-1.25 text-sm font-bold ${active.urgente ? "text-warning-strong" : "text-text-3"}`}>
          ...
          <div className="flex-1 overflow-y-auto p-4 lg:p-6">
            ...
            <div className="mb-5 grid grid-cols-2 gap-2.5 md:grid-cols-4 md:gap-3.5">
              {[
                ...
                { label: "Documentos", value: <span>{active.documentos.length}</span> },
            ...
            <div className="mb-4 rounded-card border border-border bg-surface px-5 py-4.5">
              <div className="mb-2 flex items-center gap-2">
                ...
                <h3 className="m-0 font-display text-md font-bold text-text-1">Parecer Jurídico — Art. 53, Lei 14.133/21</h3>
              </div>
              {active.parecerJuridico ? (
                <div className={`rounded-xl border px-4 py-3 ${parecerFavoravel ? "border-tint-success-border bg-tint-success-bg" : "border-tint-danger-border bg-tint-danger-bg"}`}>
                  <div className={`text-base font-bold ${parecerFavoravel ? "text-tint-success-fg" : "text-tint-danger-fg"}`}>
                    {parecerFavoravel ? "Favorável ao prosseguimento" : "Desfavorável — corrigir antes de prosseguir"}
                  </div>
                  <p className="m-0 mt-1 text-sm text-text-2">{active.parecerJuridico.comentario}</p>
                  <p className="m-0 mt-1.5 text-xs text-text-muted">
                    {active.parecerJuridico.autor} · {formatData(active.parecerJuridico.data)}
                  </p>
                </div>
              ) : active.status === "em_revisao" ? (
                <div>
                  <p className="m-0 mb-2.5 text-sm text-text-muted">
                    O controle prévio de legalidade é obrigatório antes de encaminhar ao gestor.
                  </p>
                  <Textarea
                    value={parecerTexto}
                ...
                <p className="m-0 text-sm text-text-muted">Parecer jurídico ainda não registrado.</p>
              )}
            </div>
            ...
            <div className="mb-4 rounded-card border border-border bg-surface px-5 py-4.5">
              ...
              <div className="flex flex-col">
                {active.trilha.length === 0 && <p className="m-0 text-sm text-text-muted">Sem transições registradas.</p>}
                  ...
                  <div key={i} className="flex gap-3">
                    ...
                    <div className={`flex-1 ${i < active.trilha.length - 1 ? "pb-4" : ""}`}>
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-base font-bold text-text-1">{EVENTO_LABEL[t.evento]}</span>
            ...
            {active.status === "em_revisao" || active.status === "aguardando" || active.status === "aprovado" ? (
              <div className="rounded-card border border-border bg-surface px-5 py-4.5">
                  ...
                  <div className="mt-3">
                    {!parecerFavoravel && (
                      <div className="mb-2.5">
                        <ValidationMsg type="error" msg="Registre um parecer jurídico favorável (acima) antes de encaminhar ao gestor." />
                      </div>
                    )}
                    <Button
                      size="lg"
                ...
                {active.status === "aguardando" && !retificando && !checklistOk && (
                  <p className="mt-2 text-xs text-text-muted">
                    A aprovação exige o checklist de conformidade integralmente atendido.
                  </p>
                )}
                ...
                {active.status === "aguardando" && retificando && (
                  <div className="mt-3 rounded-xl border border-violet bg-tint-violet-bg px-4 py-4">
                    <div className="mb-2.5 text-base font-bold text-tint-violet-fg">Apontamentos de Retificação</div>
                    <p className="m-0 mb-3 text-sm text-text-3">
                      Indique, por documento e seção, o que deve ser corrigido. O elaborador verá os apontamentos no editor.
                    </p>

                    {apontamentos.length > 0 && (
                      <div className="mb-3 flex flex-col gap-2">
                        {apontamentos.map((a, i) => (
                          <div key={i} className="flex items-start gap-2 rounded-md border border-border bg-surface px-3 py-2">
                            <span className={`mt-0.5 rounded-sm px-1.5 py-0.5 font-mono text-2xs font-bold ${CATALOGO[a.tipo].chip}`}>{a.tipo}</span>
                            ...
                            <button
                              type="button"
                              onClick={() => setApontamentos((prev) => prev.filter((_, j) => j !== i))}
                      ...
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                        <Dropdown
                          ...
                          options={[
                            ...
                            ...active.documentos.map((tipo) => ({ value: tipo, label: CATALOGO[tipo].titulo })),
                          ]}
                        />
                        <Input value={apSecao} onChange={(e) => setApSecao(e.target.value)} placeholder="Seção (opcional). Ex: Estimativa do Valor" />
                      </div>
                      <Textarea value={apTexto} onChange={(e) => setApTexto(e.target.value)} placeholder="Descreva a correção necessária..." rows={2} />
                      <div>
                        <Button variant="secondary" size="sm" icon={<IconPlus size={13} strokeWidth={2.5} />} disabled={apTipo === "" || apTexto.trim() === ""} onClick={adicionarApontamento}>
                    ...
                    <div className="mt-3.5 flex flex-wrap gap-2.5">
                      <Button variant="secondary" disabled={decidir.isPending} onClick={() => { setRetificando(false); setApontamentos([]) }}>
                        Cancelar
                      </Button>
                      <Button variant="dark" disabled={decidir.isPending || apontamentos.length === 0} onClick={confirmarRetificacao}>
                        {decidir.isPending ? "Registrando..." : `Confirmar Retificação (${apontamentos.length})`}
                      </Button>
                    </div>
                  </div>
                )}
                ...
                {active.status === "aprovado" && (
                  <div className="mt-3">
                    <div className="mb-2.5 rounded-xl border border-tint-success-border bg-tint-success-bg px-4 py-3 text-tint-success-fg">
                      <span className="text-base font-bold">Processo Aprovado.</span> Homologue para concluir a contratação.
                    </div>
                    <Button
                      size="lg"
                      variant="success"
              ...
              <div className="rounded-xl border border-border bg-surface px-5 py-4 text-center">
                <div className="text-lg font-bold text-text-1">
                  {active.status === "rejeitado" ? "Processo Rejeitado" : "Processo Concluído"}
                </div>
